...
class LeftAndMainSubsites extends Extension {
	...
	public function CreateSubsiteList(){
		$list = $this->Subsites();
		...
		if($list->Count() == 1) {
			$output->push(new ArrayData(array(
				...
				'ID' => 0,
				'Title' => _t('LeftAndMainSubsites.DEFAULT_SITE', 'Default site')
			)));
		}
	 ...
	 * a subsiteMenu method returning true
	 *
	 * @return ArrayList
	 */
	public function SubsiteMainMenu(){
		...
		foreach($mainMenu as $menuItem){
			...
			if(class_exists($controllerName)){
				...
				if($controller->hasMethod('subsiteMenu') 
					&& $controller->subsiteMenu()){
